...
router.post('/save_portfolio', function (req,res,next) {
    ...
    Portfolio.save_portfolio(user_id,title,label,cover_url,photo_url,width,height,depic,function (err,result) {
        if(result)
        {
...
router.post('/save_popularity', function (req,res,next) {
    ...
    let promise = new Promise(function(resolve,rejeact){
        Portfolio.get_portfolio(id,function (err,result) {
            console.log(result[0].popularity);
            count = result[0].popularity;
            resolve();
        });
    });
...
router.post('/get_rank',function (req,res,next) {
    var page =req.body.page;
    var count=2;
...
router.post('/get_recent',function (req,res,next) {
    var page =req.body.page;
    var count=2;
